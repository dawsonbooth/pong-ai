...
from constants import MOUSEMOTION, PADDLEHEIGHT, WINDOWHEIGHT, WINDOWWIDTH
    ...
    class QLearning():
        ...
        def load_state(self):
            self.state = self.discretize(
                [self.paddle.centery, self.game.ball.centery, self.game.ball.velocity.y])

        def load_distance_ball(self):
            self.distance_ball = abs(
                self.paddle.centery - self.game.ball.centery)
        ...
        def act(self):
            ...
            if (self.last_state and self.last_action):
                reward += (self.last_distance_ball -
                           self.distance_ball) - abs(self.game.ball.velocity.y)
